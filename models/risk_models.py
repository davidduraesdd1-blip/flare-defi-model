...
def estimate_il_for_pool(il_risk: str) -> tuple:
    """
    ...
    return il_table.get(il_risk, (6.0, 15.0))
...
def compute_risk_score(il_risk: str, protocol_type: str, data_freshness: str) -> float:
    """
    ...
    score = min(10.0, base + type_add + leverage_add + stale_add)
...
def build_opportunity(
    ...
    # IL calculation
    il_exp, il_worst = estimate_il_for_pool(il_risk)
    ...
    # Risk score
    rs = compute_risk_score(il_risk, protocol_type, data_source)
    ...
    # APY range (±20% around central estimate)
...
def _build_candidate_list(scan_result: dict) -> list:
    """
    ...
    for pool in scan_result.get("pools", []):
        ...
        candidates.append(build_opportunity(
            rank=rank,
            protocol_key=proto,
            pool_or_asset=pool.get("pool_name", "Unknown Pool"),
    ...
    for rate in scan_result.get("lending", []):
        ...
        candidates.append(build_opportunity(
            rank=rank,
            protocol_key=proto,
            pool_or_asset=rate.get("asset", "Unknown Asset"),
    ...
    for stake in scan_result.get("staking", []):
        ...
        candidates.append(build_opportunity(
            rank=rank,
            protocol_key=proto,
            pool_or_asset=stake.get("token", "Unknown Token"),
...
def run_conservative_model(scan_result: dict) -> list:
    candidates = _build_candidate_list(scan_result)
    # Re-score all as conservative
    for c in candidates:
        c.risk_profile = "conservative"
    return optimise_portfolio(candidates, "conservative")


def run_medium_model(scan_result: dict) -> list:
    candidates = _build_candidate_list(scan_result)
    for c in candidates:
        c.risk_profile = "medium"
    return optimise_portfolio(candidates, "medium")


def run_high_model(scan_result: dict) -> list:
    candidates = _build_candidate_list(scan_result)
    for c in candidates:
        c.risk_profile = "high"
    return optimise_portfolio(candidates, "high")
...
def run_all_models(scan_result: dict) -> dict:
    ...
    base_candidates = _build_candidate_list(scan_result)
    results = {}
